...
export class TOTP {
  ...
  static async generate(
    ...
    const timeHex = TOTP.dec2hex(
      Math.floor(epochSeconds / _options.period)
    ).padStart(16, "0");

    const keyBuffer =
      _options.encoding === "hex"
        ? TOTP.base32ToBuffer(key)
        : TOTP.asciiToBuffer(key);

    const hmacKey = await TOTP.crypto.importKey(
      "raw",
      keyBuffer,
    ...
    const signature = await TOTP.crypto.sign(
      "HMAC",
      hmacKey,
      TOTP.hex2buf(timeHex)
    );

    const signatureHex = TOTP.buf2hex(signature);
    const offset = TOTP.hex2dec(signatureHex.slice(-1)) * 2;
    const masked =
      TOTP.hex2dec(signatureHex.slice(offset, offset + 8)) & 0x7fffffff;
    const otpString = masked.toString().slice(-_options.digits);
    const otp = _options.explicitZeroPad
      ? otpString.padStart(_options.digits, "0")
      : otpString;
  ...
  private static hex2buf(hex: string): ArrayBuffer {
    ...
    for (let i = 0, j = 0; i < hex.length; i += 2, j++)
      buffer[j] = TOTP.hex2dec(hex.slice(i, i + 2));
    return buffer.buffer as ArrayBuffer;
  }
  ...
  private static readonly crypto = webcrypto.subtle;
